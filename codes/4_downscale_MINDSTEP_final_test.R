...
JOBS <- ifelse(.Platform$GUI == "RStudio",17, as.integer(args[[1]]))
dir.create("output")
...
#load("./input/targets_ACR_LUC_finalACCREU.RData")
...
for(ttt in times){
  ...
  if(nrow(priors3)!=0){
    ...
    res1 = downscale(targets = curr.targets,
                     ...
                     xmat = curr.X,
                     betas = curr.betas %>% filter(ks %in% unique(curr.X$ks)) %>% mutate(ks=as.character(ks)),
                     options = downscale_control(MAX_EXP = log(.Machine$double.xmax/1e+200)))
  }
